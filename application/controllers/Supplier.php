...
class supplier extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model('m_supplierModel');
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->m_supplierModel->countAllData();
		$config['per_page'] = 10;
		...
		$get_table = $this->m_supplierModel->getTable($config['per_page'], $start);
	...
	public function load_edit($id)
	{
		$feature_url = $this->uri->segment(1);
		$title = $this->db->get_where('m_feature', "code = '$feature_url'")->row_array();
		$result = $this->m_supplierModel->getId($id);
		$data = [
			'title' => $title['name'],
			'url' => $title['url'],
			'data' => $result
	...
	public function save()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$result = $this->m_supplierModel->insertData();
			$msg = [
				'status' => $result,
				'data' => [],
				'message' => 'Supplier berhasil ditambahkan!'
			];
			$this->session->set_flashdata('flash_success', 'Supplier berhasil ditambahkan!');
			print json_encode($msg);
		}
	}
	...
	public function edit($id)
	{
		...
		if (!$this->form_validation->run()) {
			...
			$result = $this->m_supplierModel->editData($id);
			$msg = [
				'status' => $result,
				'data' => [],
	...
	{
		$result = $this->m_supplierModel->deleteData($id);
		$msg = [
			'status' => $result,
			'data' => [],
	...
	public function get_autocomplete()
	{
		if ($this->input->get('term')) {
			$result = $this->m_supplierModel->getAutoCompleteName($this->input->get('term'));
			if (count($result) > 0) {
	...
	public function get_kelurahan_desa()
	{
		if ($this->input->get('search_data')) {
			$result = $this->m_supplierModel->getKecamatan($this->input->get('search_data'));
			if (count($result) > 0) {
				$data = [];
				foreach ($result as $row) {
					$data[] = [
	...
	public function get_kode_pos()
	{
		if ($this->input->get('m_kelurahan_desa_id')) {
			$result = $this->m_supplierModel->getKodePos($this->input->get('m_kelurahan_desa_id'));
			if (count($result) > 0) {
				foreach ($result as $row) {
