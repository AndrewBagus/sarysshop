...
class product_category extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model('m_product_categoryModel');
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->m_product_categoryModel->countAllData();
		$config['per_page'] = 10;
		...
		$get_table = $this->m_product_categoryModel->getTable($config['per_page'], $start);
	...
	public function load_edit($id)
	{
		$feature_url = $this->uri->segment(1);
		$title = $this->db->get_where('m_feature', "code = '$feature_url'")->row_array();
		$result = $this->m_product_categoryModel->getId($id);
		$data = [
			'title' => $title['name'],
			'url' => $title['url'],
			'data' => $result
	...
	public function save()
	{
		...
		if (!$this->form_validation->run()) {
			...
			$result = $this->m_product_categoryModel->insertData();
			$msg = [
				'status' => $result,
				'data' => [],
	...
	public function edit($id)
	{
		...
		if (!$this->form_validation->run()) {
			...
			$result = $this->m_product_categoryModel->editData($id);
			$msg = [
				'status' => $result,
				'data' => [],
	...
	{
		$result = $this->m_product_categoryModel->deleteData($id);
		$msg = [
			'status' => $result,
			'data' => [],
